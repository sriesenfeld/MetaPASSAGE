...
=head2 filter_reads
 ...
 Args: A list of up to 6 arguments; first 2 are non-optional. 
  arg 1: file of reads generated by MetaSim (or their translations into
    peptides);
  arg 2: an integer N: N<0 if output should contain at most one read
    per sequence in file named by arg 3; if N==0, then all reads are
    maintained (can be useful with the optional args to get a file of
    source sequences corresponding to the reads and to get ids
    converted); otherwise N is the number of reads that should be
    sampled uniformly at random from the file named by arg 1;
...
sub filter_reads ($$;$$$$) {
    ...
    # print ''.scalar(@frag_hdrs)." headers retrieved from $fragfile.\n";
	...
	# print ''.scalar(@src_hdrs)." source seq headers retrieved from $seqfile_hdrs.\n";
	$src_gene_ids_ar = get_gene_ids_from_headers(\@src_hdrs);
	# print ''.scalar(@{$src_gene_ids_ar}). " gene ids retrieved from those headers.\n";
	foreach my $gene_id (@{$src_gene_ids_ar}) {
	    # print "Considering gene id: $gene_id\n";
	    # NOTE!!: May want to change the code here since this grep could accidentally match a shorter gene id
	    # or a gene with a '.' character in it or something crazy.
	    # (Not a problem in data where all ids are same length and have only word chars, so not urgent.)
	    # (Could consider moving the below frag_id, gene_id retrieval 
	    # before this part to be able to match the gene id exactly.)
    ...
    # print ''.scalar(@chosen_frag_hdrs)." read headers selected from those matched:\n".
	# join('', @chosen_frag_hdrs)."\n";

    $chosen_frag_metasim_ids_ar = cut_to_ids(\@chosen_frag_hdrs);
    
    # print ''.scalar(@{$chosen_frag_metasim_ids_ar})." metasim ids from those headers selected:\n".
	# join(', ', @{$chosen_frag_metasim_ids_ar})."\n";
	...
	if ($linefrag =~ /$metasim_header/) {
	    ...
	    # either not a header line or not an interpretable header line
	    next;
	}
	# print "header line: $linefrag";
	# print "frag id: $frag_id; gene id: $gene_id\n";
	if (! scalar( grep {$_ eq $frag_id} @{$chosen_frag_metasim_ids_ar}) ) {
	    # print "Frag id was not selected.\n";
	    next;
	} # else {
	    # print "Matches:\n".join("\n", grep {$_ eq $frag_id} @{$chosen_frag_metasim_ids_ar})."\n";
	# }
	if (! scalar( grep {$_ eq $gene_id} @chosen_src_gene_ids) ) {
	    # print "Remembering gene id $gene_id\n";
	    push(@chosen_src_gene_ids, $gene_id);
	}
	print OUT ">$frag_id-$gene_id ";
	# print "Copying fragment for $frag_id-$gene_id\n";
